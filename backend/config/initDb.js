...
async function initializeDatabase() {
  try {
    ...
    // Check if stories table exists (our latest feature)
    const [storyTableCheck] = await pool.query(
      "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'stories'",
      [process.env.DB_NAME || 'pulse_social']
    );

    if (storyTableCheck.length === 0) {
      console.log('📦 Creating stories tables...');
