...
export default function PostCard({ post: initialPost, onDelete }) {
  ...
  const handleEdit = async () => {
    try {
      const { data } = await api.put(`/posts/${post.id}`, { content: editContent, image_url: editImageUrl });
      setPost(data);
      setEditing(false);
    } catch {}
  };
    ...
    <div className="card" style={{ marginBottom: 12, transition: 'border .2s' }}>
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        {editing ? (
          ...
          <>
            <p style={{ fontSize: 15, lineHeight: 1.65, color: 'var(--text)', marginBottom: post.image_url ? 12 : 0 }}>{post.content}</p>
            {post.image_url && (
      ...
      {/* Action bar */}
      <div style={{ borderTop: '1px solid var(--border)', padding: '10px 20px', display: 'flex', gap: 20, alignItems: 'center' }}>
